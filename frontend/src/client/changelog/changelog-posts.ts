...
export const CHANGELOG_POSTS: ChangelogPost[] = [
  {
    ...
    tags: ["feature-update"],
    summary:
      "The game details page shows what the game moved for both players: the rank on the leaderboard, the season score and rank, and the Hall of Fame score.",
    body: [
      text(
        "Each player gets a table with the value before the game, the value after it, and the change. The rows are the rank on the overall leaderboard, the score and the rank on the season leaderboard, and the Hall of Fame score. A row opens the page that shows that number.",
      ),
      text(
        "The table shows a dash where the player has no value. An unranked player has no rank. A player with no earlier game in the season starts the season score at 0. A game in the break between 2 seasons changes no season score, and the page says so.",
      ),
      text(
  ...
  },
  {
    ...
    body: [
      ...
      text(
        "A second toggle selects what the graph shows. Cumulative stacks the 9 sections as areas, with a solid color for each section. The top edge of the stack is the total score. A legend below the graph shows which color is which section. Delta stacks the same areas, with the points each section gained in each period. A section picker filters both to the total score or to 1 of the 9 sections.",
      ),
      text(
