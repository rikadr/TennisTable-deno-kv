...
export const Users: React.FC = () => {
  const query = useQuery<{ users: { username: string; role: string }[] }>({
    queryKey: ["users"],
    queryFn: async () => {
      const response = await httpClient(process.env.REACT_APP_API_BASE_URL + "/user");
          ...
          query.data.users
            ...
            .map((user) => (
              <User
                ...
                onDelete={() => deleteUserMutation.mutate({ username: user.username })}
              />
            ))}
      </div>
    </div>
...
const User: React.FC<{ username: string; role: string; onDelete: () => void; isYou: boolean }> = ({
  username,
  role,
  onDelete,
  isYou,
  ...
  return (
    <div className="w-56 bg-secondary-background text-secondary-text rounded-md m-2 flex-col flex items-center justify-between px-2 py-3">
