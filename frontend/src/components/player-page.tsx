...
export const PlayerPage: React.FC = () => {
  ...
  const reverseGames = useMemo(() => {
    ...
    return playerSummaryQuery.data.games.slice(
      Math.max(playerSummaryQuery.data.games.length - 5, 0),
    ).reverse();
            ...
            <section className="flex space-x-4 text-md">
              ...
              <div>
                ...
                {((playerSummaryQuery.data?.wins || 0) /
                  (playerSummaryQuery.data?.loss || 0)).toLocaleString(
                    "no-NO",
                    {
  ...
  if (active && payload && payload.length) {
    ...
    return (
      <div className="p-2 bg-slate-700 ring-1 ring-white rounded-lg">
        <p className="">
          {`Elo : ${
            payload[0].value?.toLocaleString("no-NO", {
              maximumFractionDigits: 0,
            })
          }`}
        </p>
